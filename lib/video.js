...
module.exports = function (filePath, settings, infoConfiguration, infoFile) {
	...
	/**
	* Add a input option to be bundled into the ffmpeg command call
	*/
	this.addInputOption = function (option, value) {
		// Check if exists the current option
		if(utils.in_array(option, inputOptions) === false) {
			//Add the new input option
			inputOptions.push(option);
			//Add the value to new option
			if( value != undefined)
				inputOptions.push(value);
		}else
			throw errors.renderError('option_already_exists', option);
	}
	...
	var execCommand = function (callback, folder) {
		...
		var finalCommands = ['ffmpeg']
			.concat(inputOptions.join(' '))
			.concat('-i')
